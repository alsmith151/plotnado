...
This guide is about the visual layer: how tracks scale, label, color, and share space. For exact field names, pair it with [Aesthetics Reference](aesthetics_reference.qmd).

The rendered panels on this page are paired with live Quarto examples in the [Living Gallery](example_coverage.qmd), where the code cells and output are rendered together from the current package.
...
![Automatic kwarg routing output](images/quickstart.png)
...
Runnable example: [BigWig styles](example_coverage.qmd#bigwig-styles)
...
Runnable example: [BED and narrowPeak](example_coverage.qmd#bed-and-narrowpeak)
...
Runnable example: [Overlay, autoscale, and highlight](example_coverage.qmd#overlay-autoscale-and-highlight)
...
::: {.callout-tip title="When overlay scaling looks wrong"}
...
Related runnable example: [Gene label strategies](example_coverage.qmd#gene-label-strategies)
...
Runnable example: [Links, hline, and vline](example_coverage.qmd#links-hline-and-vline)
